...
This wraps `CGEvent`. It is not unit-tested (it talks to the OS); verified manually in Task 12.
...
struct SettingsView: View {
    ...
    private let editableButtons: [ButtonID] = [.a, .b, .x, .y, .lb, .rb, .lt, .rt, .menu, .options]
    ...
    var body: some View {
        Form {
            ...
            Section("Buttons") {
                ForEach(editableButtons, id: \.self) { button in
                    LabeledContent(button.rawValue.uppercased()) {
                        Text(describe(model.config.mapping.buttons[button]))
                            .foregroundStyle(.secondary)
                    }
                }
                Text("Edit bindings in config.json (Application Support/Slouch). A full in-app editor is planned.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
    ...
    private func describe(_ action: OutputAction?) -> String {
        switch action {
        case .mouseClick(let b): return "\(b.rawValue) click"
        case .keystroke(let k): return "key \(k.keyCode)"
        case .sleep: return "sleep"
        case .none, nil: return "—"
        }
    }
}
```
...
## Task 12: Manual end-to-end verification on the real controller
...
## Task 13 (optional): Launch at login
